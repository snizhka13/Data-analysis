...
if app_mode == "Описова статистика":
    ...
    **1. Як визначити оптимальну кількість кластерів по дендрограмі?**
    ...
    with st.expander("**Як працює цей модуль та як розуміти результати**", expanded=False):
        st.markdown("""
        ### Суть змагання: Класифікація
        У цьому модулі чотири різні алгоритми машинного навчання змагаються у вирішенні задачі **класифікації**. Їхня мета — проаналізувати вхідні дані (ознаки об'єкта) та правильно вгадати, до якої категорії (класу) він належить. Наприклад: чи виживе пасажир, чи хворий пацієнт, до якого виду належить квітка.
        
        ---
        ### Суть процесу: Як алгоритми вчаться?
        Уявіть, що кожен алгоритм — це старанний студент. Коли ви натискаєте "Запустити аналіз", відбувається наступне:
        
        * **Розподіл на 'Підручник' та 'Іспит':** Програма не показує алгоритму всі дані одразу. Вона розрізає ваш файл на дві частини. 
            * **Навчальна вибірка (Train):** Це 70-80% даних. На них алгоритм шукає закономірності (наприклад: *"якщо у пінгвіна дзьоб довший за 45 мм, це зазвичай самець"*). 
            * **Тестова вибірка (Test):** Це решта 20-30% даних. Алгоритм їх **не бачить** під час навчання. Вони стають для нього "новими даними", на яких ми перевіряємо його кмітливість (ви можете самостійно керувати цим балансом за допомогою повзунка "Розмір тестової вибірки" у бічній панелі). 
            **Менше % (напр. 10%):** Модель отримує більше даних для навчання і може стати розумнішою. Але "іспит" буде занадто коротким, щоб об'єктивно оцінити її реальну точність.
            **Більше % (напр. 50%):** Іспит стає дуже суворим і надійним, але алгоритму банально може не вистачити даних у "підручнику", щоб знайти правильні закономірності (точність впаде).
        * **Пошук закономірностей:** Алгоритми працюють як інтелектуальні сортувальники. Вони аналізують ознаки (вагу, розміри, стать) і будують внутрішні правила для прийняття рішень.
        
        ---
        Кожен алгоритм має свій унікальний підхід до навчання:
        * **Tree (Дерево рішень):** Діє як гра "Вгадай хто?". Задає послідовні запитання до даних (наприклад, "вік > 30?", "стать чоловіча?"), розгалужуючи їх, поки не прийде до фінального рішення.
        * **Random Forest (Випадковий ліс):** Збирає "консиліум" з десятків різних дерев рішень. Кожне дерево робить свій прогноз, а фінальний результат обирається більшістю голосів. Дуже потужний і точний метод.
        * **Logistic Regression (Логістична регресія):** Використовує математику для проведення складної межі між класами. Вона розраховує ймовірність (у відсотках), з якою об'єкт належить до певного класу.
        * **Naive Bayes (Наївний Байєс):** Спирається на статистику і теорему Байєса. Називається "наївним", бо робить спрощене припущення, що всі ознаки об'єкта абсолютно незалежні одна від одної.

        ---
        ### Як інтерпретувати результати (Таблиця Test and Score)
        Алгоритми перевіряються на "тестовій" частині даних, яку вони раніше не бачили. Їхня успішність оцінюється за такими метриками:
        * **AUC (Area under ROC):** Найважливіший показник здатності моделі відрізняти класи. `1.0` — ідеально, `0.5` — випадкове вгадування. Чим ближче до 1, тим краще.
        * **CA (Classification Accuracy):** Загальна точність. Який відсоток об'єктів модель вгадала абсолютно правильно.
        * **Prec (Влучність / Precision):** Показує якість позитивних прогнозів. Скільки з тих, кого модель назвала "хворими", виявилися дійсно хворими.
        * **Recall (Повнота):** Показує здатність моделі знаходити шукане. Яку частку реально "хворих" людей модель змогла виявити.
        * **F1-score:** "Золота середина" між Влучністю та Повнотою. Особливо важливий показник, коли одного класу в даних набагато більше, ніж іншого.
        * **MCC:** Коефіцієнт кореляції Метьюса. Дуже надійна метрика, що враховує як правильні, так і всі хибні прогнози моделі (-1 — найгірше, 1 — ідеально).

       ---
        ### Матриця помилок (Confusion Matrix)
        Це детальний звіт про "помилки на іспиті".
        * **Яскраві клітинки по діагоналі:** Це ситуації, де прогноз збігся з реальністю.
        * **Інші клітинки:** Тут видно, які саме класи алгоритм плутає найчастіше (наприклад, приймає самку за самця).

        ---
        ### ROC-аналіз
        Це графік "якості" всього навчання. Чим вище лінія моделі вигинається до лівого верхнього кута, тим краще вона засвоїла закономірності у ваших даних і тим менше помилок зробить на об'єктах, яких вона ще ніколи не бачила.
        """)
    ...
    st.subheader("Попередня обробка даних (Preprocess)")
    df_ml = df_ml.dropna()

    potential_targets = []
    for col in df_ml.columns:
        is_numeric_float = pd.api.types.is_float_dtype(df_ml[col])
        unique_count = df_ml[col].nunique()

        if not is_numeric_float:
            if pd.api.types.is_object_dtype(df_ml[col]) or \
                    pd.api.types.is_categorical_dtype(df_ml[col]) or \
                    pd.api.types.is_bool_dtype(df_ml[col]) or \
                    unique_count < 15:
    ...
    if len(potential_targets) > 0:
        target_col = st.selectbox(
            "**Оберіть цільову змінну (Target):**",
            potential_targets,
            help="Для класифікації доступні лише категоріальні змінні. Числові ознаки з великою кількістю значень (як-от 'age') відфільтровані."
        )
        st.info(f"Доступно для класифікації: **{', '.join(potential_targets)}**")
    else:
        st.error("У завантаженому наборі даних не знайдено категоріальних змінних, придатних для класифікації.")
        st.stop()
    ...
    test_size = st.sidebar.slider(
        ...
        help="Визначає, який відсоток даних буде 'сховано' від алгоритму під час навчання, щоб потім використати їх як питання на іспиті. Стандартне значення: 20% - 30%. "
    ) / 100.0
    ...
    st.markdown("---")
    st.subheader("Результати оцінювання (Test and Score)")
    results_df = pd.DataFrame(results).set_index("Model")
    st.dataframe(results_df.style.format("{:.3f}").background_gradient(cmap='Blues', axis=0))

    st.markdown("---")
    st.subheader("Матриця помилок (Confusion Matrix)")
    ...
    fig_cm, ax_cm = plt.subplots(figsize=(8, 3))
    ...
    st.markdown("---")
    st.subheader("ROC-аналіз (ROC Analysis)")
    ...
    st.markdown("---")
    st.subheader("Обґрунтування результатів")
